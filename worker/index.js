...
async function article(env, request, slug) {
  ...
  const articleSchema = [
    { '@context': 'https://schema.org', '@type': 'NewsArticle', headline: item.title, description: item.meta_description || item.dek, image: [ogImage], datePublished: item.published_at, dateModified: item.modified_at, mainEntityOfPage: canonical, author: { '@type': 'Organization', name: 'Sullana Noticias' }, publisher: { '@type': 'Organization', name: 'Sullana Noticias' } },
  ...
  const extra = `<link rel="canonical" href="${esc(canonical)}"><meta property="og:type" content="article"><meta property="og:title" content="${esc(item.title)}"><meta property="og:description" content="${esc(item.meta_description || item.dek)}"><meta property="og:url" content="${esc(canonical)}"><meta property="og:image" content="${esc(ogImage)}"><meta property="article:published_time" content="${esc(item.published_at)}"><meta property="article:modified_time" content="${esc(item.modified_at)}"><script type="application/ld+json">${JSON.stringify(articleSchema)}</script>`;
