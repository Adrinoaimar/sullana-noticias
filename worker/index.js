...
function card(article) { return `<article class="card"><div class="eyebrow">${esc(article.category_name || 'Actualidad')} · ${esc(article.published_at || 'Sin fecha')}</div><h3><a href="/noticias/${encodeURIComponent(article.slug)}">${esc(article.title)}</a></h3><p>${esc(displayDek(article))}</p><a class="button" href="/noticias/${encodeURIComponent(article.slug)}">Leer noticia ↗</a></article>`; }
