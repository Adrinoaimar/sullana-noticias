...
La observación del `schedule` de `7,37 * * * *` no mostró eventos hasta las 00:08 UTC, aunque los dispatches manuales funcionan. Para recuperar el entrypoint con ejecuciones automáticas históricas, el commit siguiente devuelve ese cron directamente a `ingest.yml` y deja `scheduled-ingest.yml` como wrapper manual, sin duplicar corridas.
