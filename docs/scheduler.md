...
La periodicidad productiva se reparte en dos relojes: GitHub Actions dispara `scheduled-ingest.yml` a `17 * * * *` y `services/github-scheduler`, un Worker Cloudflare aislado, usa el patrón estándar `*/30 * * * *` como respaldo. Cada reloj mantiene un único intento por intervalo; el Worker aplica reintentos limitados y guarda una marca temporal en KV para deduplicar. `GITHUB_DISPATCH_TOKEN` vive únicamente como secreto del Worker. El disparador manual admite `publish_safe=false` para guardar un lote como borradores sin publicación masiva.
