...
La periodicidad productiva usa un solo reloj: el Worker `services/github-scheduler` ejecuta `7,37 * * * *` en UTC y despacha `workflow_dispatch` de `scheduled-ingest.yml`. GitHub Actions sigue siendo el runner Linux que instala Chromium y ejecuta Playwright; el workflow conserva una entrada manual. Así se evita depender de la entrega variable de eventos `schedule` de GitHub y no se ejecutan dos relojes. El Worker usa KV para deduplicar cada tick y reintenta el dispatch ante errores transitorios.

La verificación debe consultar los logs del Worker, el historial de Actions y `/api/health`; una corrida manual no sustituye la confirmación de un tick automático. `GITHUB_DISPATCH_TOKEN` y `SULLANA_INGEST_TOKEN` viven únicamente como secretos.
