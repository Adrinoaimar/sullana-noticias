...
To create that secret without writing a session file, install the requirements locally and run `python scripts/capture-facebook-state.py | gh secret set FACEBOOK_STORAGE_STATE_B64 --repo Adrinoaimar/sullana-noticias --body -`. The helper opens Facebook for a manual login, prints only the base64 state to the pipe, and never saves credentials in the repository or sends them to this chat. Use it only for content visible to that authorized account; it does not bypass CAPTCHA, access controls or blocks.
