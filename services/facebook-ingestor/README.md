...
The three initial sources are checked conservatively (maximum three posts, two scrolls, timeout, retries and a 30-second interval). The normalized minimum is `text`, `published_at`, `source_url` and the canonical original `post_url`; optional engagement and image fields may be null. `post_id`, URL and the existing D1 content hash provide deduplication downstream.
